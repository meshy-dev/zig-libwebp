...
/// MakefileScanner for Zig master (uses new Io API)
/// Encapsulates the Io handle and provides methods for parsing Makefile.am files
pub const MakefileScanner = struct {
    ...
    pub fn parseMakefileAm(
        ...
        // Read file using Io.Dir.cwd().readFileAlloc (Zig master API)
        const makefile_content = try Io.Dir.cwd().readFileAlloc(
            self.io,
            makefile_path,
            self.allocator,
    ...
    pub fn findSimdFiles(
        ...
        // Open directory using Io.Dir API with iterate flag
        var dir = Io.Dir.cwd().openDir(self.io, full_path, .{ .iterate = true }) catch |err| switch (err) {
